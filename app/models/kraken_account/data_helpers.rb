...
module KrakenAccount::DataHelpers
  ...
  private
    ...
    def parse_decimal(value)
      ...
      Rails.logger.error("KrakenAccount::DataHelpers - Failed to parse decimal value: #{value.inspect} - #{e.message}")
      nil
    end
    ...
    def parse_date(date_value)
      ...
      Rails.logger.error("KrakenAccount::DataHelpers - Failed to parse date: #{date_value.inspect} - #{e.message}")
      nil
    end
    ...
    def resolve_security(symbol, symbol_data = {})
      ...
      if security && security.name&.start_with?("{")
        ...
        Rails.logger.info "KrakenAccount::DataHelpers - Fixing security name: #{security.name.first(50)}... -> #{new_name}"
      ...
      Rails.logger.info "KrakenAccount::DataHelpers - Creating security: ticker=#{ticker}, name=#{security_name}"
      ...
      Rails.logger.error "KrakenAccount::DataHelpers - Failed to create security #{ticker}: #{e.message}"
